...
-- UNIQUE sur (professeur_id, annee_scolaire_id) : un prof ne peut être titulaire que d'une seule classe par an (optionnel selon vos règles).
CREATE TABLE titulaires_classes (
    id                SERIAL PRIMARY KEY,
    professeur_id     Long REFERENCES profils_professeurs(id) ON DELETE CASCADE,
    classe_id         Long REFERENCES classes(id) ON DELETE CASCADE,
    annee_scolaire_id Long REFERENCES annees_scolaires(id) ON DELETE CASCADE,
...
-- Index de performance (Section 16)
